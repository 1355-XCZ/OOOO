...
Then copy and edit the local config (optional -- defaults point to `data/` within project):
...
This downloads datasets into `data/` (final) using `download/` as temp staging. The following are **auto-downloaded**:
...
|---------|--------------|
| **IEMOCAP** | Apply at https://sail.usc.edu/iemocap/ then extract to `data/IEMOCAP_full_release/` |
| **MSP-Podcast** | Apply at https://ecs.utdallas.edu/research/researchlabs/msp-lab/MSP-Podcast.html then place in `data/MSP/` |

If you already have IEMOCAP/MSP on your machine, use `--copy-from` to copy them automatically:
...
│
├── data/                         # Datasets + models + splits (gitignored)
│   ├── ESD/, RAVDESS/, ...       #   Downloaded/placed datasets
│   ├── models/emotion2vec.../    #   emotion2vec model
│   └── splits/                   #   Generated by prepare_splits.py
├── download/                     # Temp staging for downloads (gitignored)
...
Datasets are downloaded to `data/` via `python scripts/utils/download_datasets.py --all`. IEMOCAP and MSP-Podcast require separate license applications.
